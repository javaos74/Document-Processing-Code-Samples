...
{
    internal static class UpstageOCRResultHelper
    {
        ...
        internal static async Task<OCRResult> FromUpstageClient(string file_path, Dictionary<string, object> options)
        {
            OCRResult ocrResult = new OCRResult();
            var client = new UiPathHttpClient(options["endpoint"].ToString());
            client.setAuthorizationToken(options["secret"].ToString());

            client.AddFile(file_path, "image");
            ...
            System.IO.File.WriteAllText(@"C:\Temp\upstage_resp.json", resp.body);
#endif
            if (resp.status == HttpStatusCode.OK)
            {
                StringBuilder sb = new StringBuilder();
                JObject respJson = JObject.Parse(resp.body);
                JArray blocks = (JArray)respJson["pages"][0]["words"];
                ocrResult.Words = blocks.Select(p => new Word
                {
                    Text = (string)p["text"],
                    Confidence = Convert.ToInt32(100 * ((double)p["confidence"])),
                    PolygonPoints = ((JArray)p["boundingBox"]["vertices"]).Select(v => new PointF
                    {
                        X = (float)v["x"],
                        Y = (float)v["y"]
                    }).ToArray(),
                ...
                ocrResult.Text = respJson["pages"][0]["text"].ToString();
                ocrResult.SkewAngle = 0;
                ocrResult.Confidence = Convert.ToInt32(100 * ((double)respJson["pages"][0]["confidence"]));
            }
            return ocrResult;
        }
